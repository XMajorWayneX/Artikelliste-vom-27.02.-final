...
function ItemManagement({ items, regions, onAddItem, onUpdateItem, onDeleteItem }) {
  ...
  useEffect(() => {
    // Save manual sort order to localStorage whenever it changes
    localStorage.setItem('manualSortOrder', JSON.stringify(manualSortOrder));
  }, [manualSortOrder]);
  ...
  const moveItemUp = async (item) => {
    ...
    if (currentIndex > 0) {
      ...
      // Save manual sort order to localStorage after reordering
      localStorage.setItem('manualSortOrder', JSON.stringify(newSortOrder.map(item => item.id)));
