...
function ItemSearch({ items, regions }) {
  ...
  const [manualSortOrder, setManualSortOrder] = useState([]); // Add manualSortOrder state
  const [filteredAndSortedItems, setFilteredAndSortedItems] = useState([]);

  useEffect(() => {
    // Load manual sort order from localStorage when the component mounts
    const storedSortOrder = localStorage.getItem('manualSortOrder');
    if (storedSortOrder) {
      setManualSortOrder(JSON.parse(storedSortOrder));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Apply manual sort order if available
    let sortedItems = [...filteredItems];
    if (manualSortOrder.length > 0) {
      sortedItems = manualSortOrder
        .map(id => filteredItems.find(item => item.id === id))
        .filter(item => item !== undefined);
    } else {
      // Default sort by the first three letters
  ...
  }, [items, searchTerm, selectedRegion, schutzartFilter, bwsFilter, typFilter, artFilter, serieFilter, materialFilter, manualSortOrder]);
